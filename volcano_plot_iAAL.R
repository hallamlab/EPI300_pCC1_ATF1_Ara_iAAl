# ============================================================
# Volcano plot: EPI300_pCC1_ATF1 (iAAL vs noAra)
...
df <- read_csv("EPI300_pCC1_ATF1_noAraiAAlCla.csv", show_col_types = FALSE) %>%
  filter(Name != "0", !is.na(EPI300_pCC1_ATF1_noAra), !is.na(EPI300_pCC1_ATF1_iAAL))
...
# share of the total signal in iAAL vs noAra. Treat these p-values as a
...
p <- ggplot(df, aes(x = log2FC, y = neglog10p)) +
  ...
  coord_cartesian(xlim = c(-6, 6), ylim = c(0, 1e6)) +
  scale_y_continuous(
